...
function renderSocialLinks(data: EmailData): string {
  const active = data.socialLinks.filter(s => s.enabled);
  if (!data.socialEnabled || !active.length) return '';

  const textAlign = data.socialAlignment === 'center' ? 'center' : data.socialAlignment === 'right' ? 'right' : 'left';
  const icons = active.map(s => {
    const icon = SOCIAL_ICONS[s.platform] || '';
    const href = s.url || '#';
    return `<a href="${href}" style="display:inline-flex;width:36px;height:36px;align-items:center;justify-content:center;background:#1a1a1a;border-radius:50%;margin:0 4px;text-decoration:none;vertical-align:middle;">${icon}</a>`;
  }).join('');

  return `<div style="padding:16px 0 8px;text-align:${textAlign};">${icons}</div>`;
}
...
export function renderPreviewHTML(data: EmailData): string {
  ...
  const paragraphsHtml = data.bodyParagraphs
    ...
    .map(p => `<p style="margin:0 0 16px;color:#1a1a1a;font-size:${p.fontSize ?? 16}px;line-height:1.7;font-family:Inter,-apple-system,sans-serif;${alignStyle(p.align)}">${p.html}</p>`)
    .join('');
